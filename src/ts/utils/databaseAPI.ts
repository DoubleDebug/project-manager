...
export class DatabaseAPI {
  ...
  // using RxJS
  static getProjectsByText2(text: string): Observable<any> {
    const projectsObs = ajax(`${DatabaseAPI.rootURL}/projects?q=${text}`);
    const tasksObs = ajax(`${DatabaseAPI.rootURL}/tasks`);

    return zip(projectsObs, tasksObs).pipe(
      map((res) => {
        const projects = res[0].response as any[];
        const tasks = res[1].response as any[];

        return projects
          .map((p: any) => new Project(p.id, p.name, p.dueDate, p.timestamp))
          .map((p: Project) => {
            const projectTasks = tasks
              .filter((t: any) => t.projectId === p.model.getId())
              .map((t: any) => new Task(t.id, t.name, t.state));
