...
export class ManagerView {
  ...
  drawProfileHeader(model: ManagerModel) {
    ...
    const obsSearchBar = fromEvent(
      document.getElementById('inputSearch'),
      'input'
    );
    const obsNewProjectClick = fromEvent(btnAddProject, 'click');
    obsNewProjectClick.pipe(mergeWith(obsSearchBar)).subscribe(() => {
      document.getElementsByClassName(
        'lastActive'
      )[0].innerHTML = `last active: ${new Date().toLocaleTimeString()}`;
    });
  }
  ...
  drawSearchBar(parent: HTMLElement, model: ManagerModel) {
    ...
    fromEvent(inputSearch, 'input')
      ...
      .subscribe((projects: Project | Project[]) => {
        if (Array.isArray(projects)) this.drawDashboard(projects, model, false);
        else this.drawDashboard([projects], model, false);
      });
  }
  ...
  drawProfileDropdown(parent: HTMLElement, model: ManagerModel) {
    ...
    nicknameItem.innerHTML = `#${user.model.getId()} | ${
      user.model.nickname
    } <br /><span class="lastActive">(last active: ${new Date().toLocaleTimeString()})</span>`;
    ...
    btn.onclick = () => {
      ...
      if (state) {
        this.redrawDashboard(
          user
            .getProjects()
            .filter((p: Project) => p.model.getProjectState() === state),
          model
        );
  ...
  changeSortingMethod(
    ...
    const filterDropdownItems =
      document.getElementById('btnFilterDropdown').parentElement.children[0]
        .children;
